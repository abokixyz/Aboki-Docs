...
Integration Flow
Get quote first to show customer exact amounts
Verify bank account (optional but recommended)
Create order - generates unique deposit wallet
Customer sends tokens to the generated wallet address
Automatic processing - NGN sent to bank account
Webhooks notify you of status changes

Important Notes

Each order generates a unique wallet address
Customer must send the EXACT token amount specified
Orders expire after 24 hours if no deposit received
Bank account verification happens automatically during order creation
